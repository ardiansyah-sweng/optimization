...
class Raoptimizer
{
    ...
    function averageWeights($weights, $factor)
    {
        ...
        if (!isset($factor)) {
            return new Exception("Factor is empty!");
        }

        if ($factor === 'friction_factors_weights') {
            foreach ($weights as $weight) {
                $process['ff_team_composition'][] = $weight['friction_factors_weights']['A']['ff_team_composition'];
                $process['ff_process'][] = $weight['friction_factors_weights']['A']['ff_process'];
                $process['ff_environmental_factors'][] = $weight['friction_factors_weights']['A']['ff_environmental_factors'];
                $process['ff_team_dynamics'][] = $weight['friction_factors_weights']['A']['ff_team_dynamics'];
            }
        }
        if ($factor === 'dynamic_force_factor_weights') {
            foreach ($weights as $weight) {
                $process['dff_expected_team_change'][] = $weight[$factor]['A']['dff_expected_team_change'];
                $process['dff_introduction_new_tools'][] = $weight[$factor]['A']['dff_introduction_new_tools'];
                $process['dff_vendor_defect'][] = $weight[$factor]['A']['dff_vendor_defect'];
                $process['dff_team_member_responsibility'][] = $weight[$factor]['A']['dff_team_member_responsibility'];
                $process['dff_personal_issue'][] = $weight[$factor]['A']['dff_personal_issue'];
                $process['dff_expected_delay'][] = $weight[$factor]['A']['dff_expected_delay'];
                $process['dff_expected_ambiguity'][] = $weight[$factor]['A']['dff_expected_ambiguity'];
                $process['dff_expected_change'][] = $weight[$factor]['A']['dff_expected_change'];
                $process['dff_expected_relocation'][] = $weight[$factor]['A']['dff_expected_relocation'];
            }
        }
    ...
    function uniformInitializationFF()
    {
        ...
        for ($i = 1; $i <= $n - 1; $i++) {
            $R[$i] = $this->frictionFactorsRandomWeight();
        }
    ...
    function uniformInitializationDFF()
    {
        $n = $this->parameters['particle_size'];
        ...
        for ($i = 1; $i <= $n - 1; $i++) {
            $R[$i] = $this->dynamicForceFactorsRandomWeight();
        }
        ...
        foreach ($R as $key => $r) {
            $A['dynamic_force_factors'] = [
                ...
                'dff_expected_relocation' => $X1['dff_expected_relocation'] + $r['dff_expected_relocation'] / $n * ($this->parameters['dynamic_force_factors']['max'] -  $this->parameters['dynamic_force_factors']['dff_expected_relocation']),
            ];
        ...
        for ($generation = 0; $generation <= $this->parameters['maximum_generation']; $generation++) {
            ...
            if ($generation === 0) {
                ...
                for ($i = 0; $i <= $this->parameters['particle_size'] - 1; $i++) {
                    $friction_factor_weights = $this->uniformInitializationFF()[$i];
                    $dynamic_force_factor_weights = $this->uniformInitializationDFF()[$i];
            ...
            if ($generation > 0) {
                ...
                foreach ($particles[$generation] as $i => $individu) {
                    ...
                    foreach ($vel[$generation + 1]['dynamic_force_factor_weights'] as $key => $velocities) {
                        $positions_dff[$key] = $particles[$generation][$i]['dynamic_force_factor_weights']['A'][$key] + $velocities;
                    }
